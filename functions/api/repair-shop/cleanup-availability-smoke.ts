...
const TARGET_EMAIL = "repair-availability-production-smoke@hermesconnect.app";
...
export async function onRequestPost({ env }: { env: Env }) {
  ...
  const specialist = await env.DB
    .prepare("SELECT id FROM specialists WHERE email = ? LIMIT 1")
    .bind(TARGET_EMAIL)
    .first();
  ...
  await env.DB
    .prepare("DELETE FROM repair_shop_availability WHERE shop_id IN (SELECT id FROM repair_shops WHERE owner_specialist_id = ?)")
    .bind(id)
    .run();
  ...
  const remaining = await env.DB
    .prepare("SELECT COUNT(*) AS count FROM specialists WHERE email = ?")
    .bind(TARGET_EMAIL)
    .first();
