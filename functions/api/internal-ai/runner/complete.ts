...
import { ensureInternalAiSchema, internalAiTaskBranch, INTERNAL_AI_ORGANIZATION_SCOPE, nowIso, publicTask, requireInternalAiRunner, sanitizeTaskResult } from "../../_lib/internal-ai.mjs";
...
export async function onRequestPost({ request, env }: { request: Request; env: Env }) {
  ...
  const result = sanitizeTaskResult(payload); const finalStatus = existing.cancel_requested && result.status !== "failed" ? "cancelled" : result.status;
  ...
  const expectedBranch = internalAiTaskBranch(taskId);
  if (result.branch && result.branch !== expectedBranch) return jsonResponse(409, { success: false, error: "task_branch_mismatch" });
  if (finalStatus !== "failed" && result.branch !== expectedBranch) return jsonResponse(409, { success: false, error: "task_branch_required" });
  const now = nowIso();
  const update = await env.DB.prepare(`UPDATE hermes_internal_ai_tasks SET status = ?, completed_at = ?, updated_at = ?, repo_sha = ?, branch = ?, pr_url = ?, evidence_class = ?, output_summary = ?, approval_gate = CASE WHEN ? = 'needs_approval' THEN ? ELSE approval_gate END, approval_granted_at = CASE WHEN ? = 'needs_approval' THEN NULL ELSE approval_granted_at END, approval_granted_by = CASE WHEN ? = 'needs_approval' THEN NULL ELSE approval_granted_by END WHERE id = ? AND organization_scope = ? AND runner_id = 'internal-ai-mac-runner' AND status = 'running'`).bind(finalStatus, now, now, result.repo_sha, result.branch, result.pr_url, result.evidence_class, result.output_summary, finalStatus, result.approval_gate, finalStatus, finalStatus, taskId, INTERNAL_AI_ORGANIZATION_SCOPE).run();
