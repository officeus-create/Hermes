...
const US_REGION_PATTERN = new RegExp(`(?:^|[,\\s])(?:${US_REGION_CODES.join("|")})(?:$|[,\\s\\d])`, "i");
...
const hasUsLocationSignal = (address: string) => {
  ...
  return /\b(?:USA|UNITED STATES)\b/.test(upper)
    || /\b\d{5}(?:-\d{4})?\b/.test(upper)
    || US_REGION_PATTERN.test(upper);
};
...
const waypointFrom = (value: unknown): { waypoint?: { placeId: string } | { address: string }; error?: string } => {
  ...
  if (placeId) {
    if (!/^[A-Za-z0-9_-]{10,512}$/.test(placeId)) return { error: "invalid_place_id" };
...
const sameWaypoint = (left: { placeId: string } | { address: string }, right: { placeId: string } | { address: string }) => {
  if ("placeId" in left && "placeId" in right) return left.placeId === right.placeId;
  if ("address" in left && "address" in right) return left.address.toLowerCase() === right.address.toLowerCase();
  return false;
};
...
export async function handleRouteEstimate(
  ...
  if (sameWaypoint(origin.waypoint, destination.waypoint)) {
  ...
  if (!upstream.ok) {
    ...
    return json(allowedOrigin, status, {
      success: false,
      error: status === 422 ? "route_not_found" : "estimate_temporarily_unavailable",
    });
  }
