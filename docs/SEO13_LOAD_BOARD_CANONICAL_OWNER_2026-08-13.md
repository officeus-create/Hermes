...
A build-output regression test locks two properties:

1. the rendered Load Board exposes `/load-board/` as the canonical search owner;
...
An earlier experimental pass removed `?role=` / `equipment=` parameters from internal links. Browser CI showed those parameters are part of the intended role-specific product experience. That experiment was rejected rather than weakening UX to satisfy an SEO theory.
...
- no doorway URLs for wording variants.
