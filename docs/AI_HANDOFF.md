...
- Authenticated evidence: finalized GSC SEO Planner 2026-08-27..2026-09-23 reports 7 clicks / 305 impressions / 2.30% CTR / avg position 46.56 versus 31 / 1,660 / 1.87% / 40.29. U.S. lost 810 impressions; desktop lost 1,013; CTR did not collapse.
- Delivered: a 292-URL sitemap survival inventory; current visibility-loss forensic with query-family, country, device and owner classification; five privacy-gated original-evidence asset designs; current authority-registry reconciliation; and a regression gate in the canonical test chain.
- Decision safety: 32 URLs are observed in returned GSC top/query×page rows; 260 remain UNKNOWN rather than zero. No MERGE, NOINDEX or DELETE decision is made. Logistics SEO stays KEEP/observation freeze; Load Board is IMPROVE without new owners; Appleton remains PENDING_EVIDENCE with warehousing held as intent mismatch.
- Entity/authority: #204/#515 external corrections were sent but stale fields remain publicly visible; do not add those profiles to sameAs. NIADA selective pilot is declined; paid distribution is not earned authority and requires separate acquisition ROI approval.
- Verification at write: in-memory schema/invariant validation passed for 292 unique rows, zero destructive decisions and all three protected owner decisions. Exact-head build, full test and Playwright remain GitHub CI gates after the handoff commit.
- Ecosystem compounding: search measurement, entity routing, authority quality, first-party evidence planning, privacy controls and future chart/video/transcript reuse improve; no public copy, URL, schema, canonical, analytics payload, outreach, customer data, merge or deploy action was taken.
- Reusable lesson: absence from returned top rows is `UNKNOWN`, not zero; before destructive survival decisions, require a full page export or exact URL Inspection plus an adequate observation window. Country and device slices must remain separate unless the platform returns the intersection.
- Next owner/gap: CI/review for #1496, then source-level external-profile propagation proof (#204/#515) and the first permissioned original-evidence sample. GA4/Bing/Copilot/qualified-opportunity/revenue evidence remains separate.
