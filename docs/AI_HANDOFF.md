...
## 2026-08-26 — Codex — Hermes Connect AI Assistant cabinet decision

- Agent/task owner: Codex; Wave 2 AI Assistant discovery and product-boundary correction.
- Branch: `docs/hermes-connect-ai-cabinet-decision-2026-08-26` from current `origin/main` `b703dbd68a695b0d3cddacc23b4716eb035f1107`.
- Files changed: `docs/HERMES_CONNECT_AI_ASSISTANT_CABINET_DECISION_2026-08-26.md`; this handoff.
- Evidence: current `main` has one authenticated Repair Shop customer-owner workspace, unauthenticated visual demos, Academy-specific private surfaces, and a public reference-only AI Command Center. PR #865 (`c90f7dae0651d5de2b15e497d0430a78f8c88bd2`) introduces a separate stale `/services/hermes-connect/owner/` control center, so it cannot be treated as the existing cabinet.
- Decision: no runtime UI/backend was copied, merged, deployed, or exposed. PR #865 is technical reference only: its queue, runner, event, cancellation, isolation, and sanitized-evidence patterns can be rebuilt from current main only after a canonical internal owner shell and authorization contract are identified.
- Blocker: `CURRENT_CABINET_LOCATION = NOT_PROVEN_ON_CURRENT_MAIN`; using Repair Shops would violate the owner-only/internal requirement and expose the capability to customer accounts. An owner decision must identify the actual existing internal cabinet route or explicitly authorize a protected shell and its authorization/retention contract.
- Ecosystem compounding scorecard: search/conversion/internal linking neutral (no public route/CTA/canonical change); durable knowledge and reusable architecture high (prevents a duplicate product and preserves safe recovery candidates); privacy/security high (no customer-scope escalation, secret configuration, external action, or remote shell).
- Verification: `git diff --check`, `npm run build`, and `npm test` passed on this branch. Full `npm run test:e2e` completed `1044 passed / 1 failed / 11 skipped`; the unchanged desktop `business-lead-form` consent-checkbox assertion failed only in the 6-worker suite. Focused serial reproduction passed `5/5`. No assertion or runtime behavior was changed for this documentation-only decision. No UI screenshot is applicable.
- Recommended next task/owner: owner identifies the canonical authenticated internal Hermes Connect cabinet and internal authorization model; then Codex rebuilds one bounded native `AI Assistant` section plus fresh queue/runner contracts on current main.
