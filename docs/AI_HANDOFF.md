...
## 2026-09-11 — Codex — Academy worldwide Russian/Ukrainian discovery

- Agent/task owner: Codex; bounded public Academy SEO/GEO implementation on `feat/academy-global-ru-uk-growth-20260911` from current `origin/main`.
- Public outcome: the canonical Academy direction now states that Russian-speaking, Ukrainian-speaking, and English-speaking adults may apply worldwide across the five existing tracks. The copy preserves human review, current-capacity review, legal/sanctions review, email-only coordination, and explicit no-guarantee boundaries.
- Structured discovery: Academy `Service` schema now uses `Worldwide` as the served place and declares Russian, Ukrainian, and English. Three visible FAQs answer worldwide eligibility, application language, and the information an international applicant should prepare.
- Evidence correction: the governed Academy AI-visibility prompt now describes the five public learning tracks instead of the retired two-program statement. The observation ledger remains an empty baseline; no visibility, citation, applicant-country, lead, or revenue outcome is claimed.
- Crawl boundary: source `robots.txt`, Cloudflare configuration, sitemap ownership, route inventory, and URL families are unchanged. No country doorway pages, directory profiles, analytics settings, Telegram messages, deployment, or external account writes were created by this branch.
- Ecosystem compounding scorecard: search and AI-answer clarity improve on one canonical owner; conversion improves through clearer worldwide qualification into the existing application; expertise and internal linking are preserved; durable knowledge improves through contract and browser coverage; privacy remains country-level application context behind human review; reusable architecture improves through shared schema rather than country duplication; adjacent Hermes Connect products are unchanged.
- Verification: `git diff --check`; `npm run build` passed with 273 Astro routes; full `npm test` passed; dedicated-port Academy Playwright suite passed `116/116` across desktop and mobile. An earlier default-port full-browser attempt was invalidated because Playwright reused an unrelated stale preview server on port 4321; no result from that attempt is treated as branch evidence.
- Remaining gates: exact-head GitHub CI, owner review, merge, and deployment. After production is live, verify the canonical page with AI search/user agents and begin measurement from the honest empty baseline before considering country-specific expansion.
