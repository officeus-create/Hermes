...
## 2026-09-16 — Codex — removed Work.ua vacancy fail-closed preview

- Owner/executor: Site Hermes; bounded HR → Site handoff on branch `fix/pause-removed-dispatcher-job-20260916` from current main `43f2ea6d549b37de692b85a5a0708cbde6ab9ce0`.
- Verified defect: Work.ua vacancy `7362244` redirects to a search route with `job_removed=1` and visibly says the vacancy was removed or hidden. Production still says `Verified open`, exposes two Work.ua CTAs, emits `JobPosting`, and calls the Hermes form preview-only even though the form POSTs/PUTs to the private HR API and issues a server receipt.
- Preview change: dispatcher status is `paused`; owner publication approval is false; the role page is `noindex,follow` with BreadcrumbList only; Work.ua and Hermes-apply CTAs, careers-hub discovery, and sitemap discovery are removed. The generic application route now states its real persistence boundary and blocks the paused role before sending candidate data.
- Data contract: shared role policy separates general interest, approved public roles, and private HR pilot classifications. The candidate API validates `role_id`, track, classification, acceptance state, and replay consistency before persistence. Academy, customer leads, paid activation, signing, and general contact are explicitly not job intake.
- Monitor: external vacancy lifecycle treats removed markers, redirect drift, non-2xx responses, and request failures as `review_required`; paused production can pass only with zero JobPosting/CTA/hub/sitemap exposure.
- Evidence boundary: build and focused local mock browser/API tests are required; no live candidate, reviewer action, retention/deletion exercise, merge, deploy, or public write is authorized by this change. PR #1327 separately promotes the paused role from Academy and must be reconciled by its owner before merge.
