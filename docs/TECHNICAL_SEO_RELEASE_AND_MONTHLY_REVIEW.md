...
Use one evidence-first gate for every new indexable route and one repeatable monthly review for the full public site. This document does not authorize DNS, Cloudflare, billing, Search Console account changes, or publication of unsupported claims.

## A. New indexable route release checklist

Complete every item before recommending a route for release.
...
After release, record without exposing account identifiers:
