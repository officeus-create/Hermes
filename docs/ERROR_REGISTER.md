...
| ERR-SEC-001 | OWNER_ACTION | Historical Hermes Connect credential exposure | A read-only 2026-08-25 recovery audit found a plaintext Telegram credential in a historical Antigravity/legacy prototype source. The value is deliberately not recorded in this repository. | Owner must rotate/revoke the affected provider credential, verify any current consumer privately, and preserve only a redacted recovery record. | Do not copy, use, paste, commit, transmit, or report the credential value; do not treat historical config as canonical runtime. |
