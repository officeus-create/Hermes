...
| ERR-HR-001 | RESOLVED_IN_REVIEW | Public recruiting lifecycle | Work.ua vacancy 7362244 redirects to a removed/hidden marker while production still advertises it as verified open and incorrectly describes the persisted Hermes HR intake as preview-only. | Review the bounded pause PR: zero verified-open roles, no JobPosting/Work.ua CTA/hub or sitemap discovery, explicit persistence copy, server-enforced role ID classification, and fail-closed external lifecycle. Merge/deploy and production readback remain separately owner-approved. | Do not reopen the role, use another Hermes form as job intake, or claim reviewer SLA/retention/deletion proof without a verified route and separate approval. |
