...
| ERR-AI-002 | OWNER_ACTION | PR #861 workflow update transport | The completed rebased PR #861 cannot be pushed with the current GitHub OAuth token because GitHub rejects updates to `.github/workflows/codex-hermes-router.yml` without the `workflow` scope. Local source, clean-path bootstrap, build, unit suite, and E2E suite are complete; no remote branch update was accepted. | Use an owner-approved GitHub credential with the minimal `workflow` permission, then push the already verified branch with `--force-with-lease`, wait for exact-head CI, and review/merge. | Do not remove the workflow contract merely to make the push succeed, broaden credentials beyond the needed scope, expose a token, or merge the stale remote head. |
...
| ERR-RES-024 | RESOLVED | Intel macOS Hermes FCC bootstrap | Intel setup could consume a pre-existing isolated OpenSSL directory without recording a reproducible source, hash, pinned version, or an early failure when the dependency was absent or unsuitable. | `bootstrap-hermes-openssl.sh` now downloads only official OpenSSL 3.5.4, verifies SHA-256, builds static artifacts in the isolated Hermes directory, rejects system/package-manager paths, and verifies headers, static libraries, and the pinned `openssl version` before FCC installation. A mismatched existing OpenSSL fails closed. |
