...
| --- | --- | --- | --- | --- | --- |
| ERR-TG-CLOUD-001 | OWNER_ACTION | Telegram Mac retirement / group reports | Existing ProgressoPro group bot's old outbound sender was Mac-hosted; cloud replacement draft PR #1477 has offline contract tests only. No current Cloudflare deployment, webhook, group receipt, producer or token rotation proof. | Close #932 privately, authorize existing D1/Cloudflare secret and deploy path, wire approved producer, confirm identity and permissions, perform owner/group canary with `message_id` plus independent readback; track in #1293. | Do not claim other bots or TaskNotify migrated; do not duplicate polling owner, publish credentials, bypass approval/pause or retry ambiguous sends. |
