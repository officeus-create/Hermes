...
| ERR-SEO-004 | RESOLVED_IN_REVIEW | Structured data eligibility | Authenticated Semrush Site Audit dated 2026-09-04 reported 17 invalid structured-data items. The repeated cause was a `WebApplication` entity without `offers`, `review`, or `aggregateRating` on the Hermes Connect hub, reference capability pages, and the carrier resource hub. | Review the bounded schema branch: add truthful zero-price Offers to the live setup offer and free calculators, and model informational capability ownership as a WebPage. After merge/deploy, rerun the audit and verify the affected URLs. | Do not invent ratings, reviews, paid prices, availability, or treat an informational preview as a released application. |
