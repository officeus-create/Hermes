...
| ERR-AI-001 | OWNER_ACTION | Hermes Connect internal AI Assistant | Internal-only task/API/runner contract is implemented on a review branch, but no internal-owner capability binding, runner token, or outbound Mac runner has been provisioned or production-proven. `REMOTE_BROWSER_TO_CODEX = UNVERIFIED`. | In approved private admin/provider surfaces, provision a specific internal-owner capability record and scoped runner token; then execute a controlled non-production task and verify sanitized receipt/tenant isolation. | Do not commit credentials, use a customer role as an internal role, expose the route in customer navigation, enable remote shell behavior, merge/deploy automatically, or claim live runner access. |
