...
| ERR-EXT-006 | ACTIVE | GA4 production measurement | The existing Hermes property and production web stream remain authenticated. A 2026-08-29 current-main audit found that `commercial_cta_click`, `seo_intake_start`, `seo_intake_preview_ready`, and `seo_handoff_ready` were pushed only as plain `dataLayer` objects; the SEO handlers did not invoke the Google tag event command, so receipt by GA4 was not established. The bounded branch `fix/seo-ga4-event-transport-2026-08-29` adds consent-gated transport and automated exact-once/no-PII coverage. | Review the bounded PR and exact-head CI. After an approved merge/deploy, run one synthetic non-private funnel and verify one receipt per event in GA4 DebugView/Realtime before changing key-event configuration. | Do not create a replacement property, mark unused `purchase` as proof, or send names, email, phone, URLs, free text, or other submitted values to analytics. |
...
| ERR-SEO-003 | WATCH | GSC crawl/index quality | PR #888 is merged on current main. The 2026-08-29 built-output audit confirms 115 unique indexable URLs across seven sitemap files, while the nine low-evidence guided equipment routes remain available with self-canonicals, `noindex,follow`, and no sitemap membership. Repository state is correct; Google recrawl and index-state change are not yet proven. | Monitor the comparable GSC coverage/index-state after recrawl and preserve the 19 distinct public owners. | Do not bulk-request indexing, rewrite protected winners, or add superficial text to make thin variants look unique. |
| ERR-EXT-007 | OWNER_ACTION | `www` custom domain | GSC exposed the HTTP and HTTPS `www` homepage variants as blocked. A direct production check on 2026-08-28 returned Cloudflare `520` for both instead of an apex redirect. | In the authenticated Cloudflare/domain configuration, verify the `www` hostname and configure a permanent redirect to `https://hermeslogisticsus.com/`; then verify both protocols before starting GSC validation. | Do not change apex production ownership, DNS broadly, or start GSC validation while `www` still fails. |
