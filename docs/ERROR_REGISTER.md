...
| ERR-CONTACT-001 | ACTIVE | Shared public contact form delivery | On 2026-09-16, production `/paths/logistics/` switched the form to live `/api/logistics-lead`, but its published ContactCTA script parsed that relative endpoint with `new URL(endpoint)` and stopped before `fetch`. The direct API smoke bypassed this browser path. | Review the isolated relative-URL validation fix and mocked desktop/mobile 2xx/503 tests; after an authorized release, read back the production asset and obtain a separately approved non-PII receiver/owner receipt. | Do not count API smoke or a local browser mock as a real user inquiry, owner inbox receipt, qualified lead, or sale; do not send a production form without the approved QA policy. |
