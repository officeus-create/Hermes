...
| ERR-INS-001 | ACTIVE | Insights hub structured data | On `origin/main` `1596c32b` (2026-09-16), hub `ItemList` includes a brief URL that has no generated article route; sitemap and RSS already filter to standalone content. | Review the bounded `fix/insights-itemlist-standalone-20260916` branch, verify its exact-head build/static/browser checks, then separately approve merge/deploy and read back production JSON-LD. | Do not create a thin brief route or treat a review-branch fix as live. |
