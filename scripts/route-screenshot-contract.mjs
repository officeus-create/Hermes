...
export function parseScreenshotBaseUrl(value, { allowRemote = false } = {}) {
  ...
  if (!new Set(["http:", "https:"]).has(url.protocol)) {
    throw new Error(`Unsupported screenshot protocol: ${url.protocol}`);
  }
  if (url.username || url.password) {
    throw new Error("Screenshot base URL must not contain credentials");
  }
  if (!allowRemote && !LOOPBACK_HOSTS.has(url.hostname)) {
    throw new Error(
      `Remote screenshot capture is disabled for ${url.hostname}. ` +
        "Use a loopback preview or explicitly set ALLOW_REMOTE_SCREENSHOTS=true.",
    );
  }
  url.hash = "";
  url.search = "";
...
export function validateScreenshotRoutes(routes = DEFAULT_SCREENSHOT_ROUTES) {
  ...
  for (const route of routes) {
    if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(route.id)) {
      throw new Error(`Invalid screenshot route id: ${route.id}`);
    }
    if (!route.path.startsWith("/") || route.path.includes("?") || route.path.includes("#")) {
      throw new Error(`Screenshot route must be a clean absolute path: ${route.path}`);
    }
    if (ids.has(route.id) || paths.has(route.path)) {
      throw new Error(`Duplicate screenshot route: ${route.id} ${route.path}`);
    }
    ids.add(route.id);
...
export function screenshotFileName(routeId, viewportId) {
  if (!/^[a-z0-9-]+$/.test(routeId) || !/^[a-z0-9-]+$/.test(viewportId)) {
    throw new Error("Screenshot file identifiers must be slug-safe");
  }
