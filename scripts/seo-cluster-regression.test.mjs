...
const parseJsonLd = (html, route) => {
  ...
  for (const [index, block] of [...html.matchAll(/<script\b[^>]*type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)].entries()) {
    try {
      ...
      errors.push(`${route}: JSON-LD block ${index + 1} is invalid: ${error.message}`);
    }
  }
  return parsed;
};
...
for (const sitemapFile of sitemapFiles) {
  ...
  for (const url of urls) {
    ...
    try {
      ...
      errors.push(`/${sitemapFile}: invalid absolute URL ${url}`);
      continue;
    }
...
for (const url of clusterUrls) {
  ...
  const isWebApplication = route === "/logistics/resources/rpm-calculator/";
  let html;
  try {
    html = await readFile(join(dist, htmlPath), "utf8");
  } catch {
    errors.push(`${route}: sitemap URL has no generated HTML page at ${htmlPath}`);
    continue;
  }
...
for (const [title, routes] of titleOwners) {
  if (routes.length > 1) errors.push(`${routes.join(", ")}: duplicate cluster title ${title}`);
}
for (const [description, routes] of descriptionOwners) {
  if (routes.length > 1) errors.push(`${routes.join(", ")}: duplicate cluster meta description ${description}`);
}
