...
assert.match(runtime, /salesperson_code/);
assert.match(runtime, /Private salesperson code:/);
...
assert.doesNotMatch(runtime, /dataLayer\.push\([\s\S]{0,500}(contactName|contactEmail|contactPhone|salespersonCode|cityState)/);
assert.match(leadReceiver, /\["Hermes Logistics",\s*"GENERAL CONTACT \/ LOGISTICS"\]/);
assert.match(leadReceiver, /input\.consent !== true/);

console.log("Repair Shop partner offer live delivery, human-review boundary, private attribution, and zero-PII telemetry contract passed.");
