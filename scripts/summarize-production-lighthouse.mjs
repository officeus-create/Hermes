...
const result = {
  ...
  methodology: {
    lighthouseVersion: mobile.runs[0]?.lighthouseVersion ?? desktop.runs[0]?.lighthouseVersion ?? null,
    mobileRuns: mobile.runs.length,
    desktopRuns: desktop.runs.length,
    statistic: "median",
    note: "GitHub-hosted Lighthouse lab runs. Compare directionally with the owner-supplied 2026-08-11 PSI baseline; these are not CrUX field metrics and do not guarantee identical PSI infrastructure conditions.",
  },
  baseline,
  mobile,
  desktop,
...
const lines = [
  ...
  `- Statistic: median of ${mobile.runs.length} mobile / ${desktop.runs.length} desktop runs`,
  "- Evidence class: lab only; not CrUX field data",
  "",
  "## Mobile median",
  "",
  ...
  "",
  "> This lab comparison is repeatable and useful for regression direction, but it is not a substitute for PageSpeed/CrUX field evidence. A score change alone is not a ranking guarantee.",
  "",
];
