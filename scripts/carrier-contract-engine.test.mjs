...
import { onRequest, normalizeInput, parseAllowedPercentages } from "../functions/api/carrier-contract.ts";
...
const reviewMasterPath = "/contracts/Hermes_Carrier_Administrative_and_Dispatch_Support_Agreement_v3_ATTORNEY_REVIEW.pdf";
const reviewMaster = await readFile(new URL(`../public${reviewMasterPath}`, import.meta.url));
const reviewMasterSha = sha256(reviewMaster);
assert.equal(reviewMasterSha, "9d26436b95b63610179f3af9ac4cddf5df59a1610e402bad2162ef394951d5cb");
...
const env = {
  ...
  ASSETS: {
    async fetch(input) {
      ...
      assert.equal(url.pathname, reviewMasterPath);
      return new Response(reviewMaster, { status: 200, headers: { "Content-Type": "application/pdf" } });
    },
  },
...
const validPayload = {
  ...
  company_website: "https://carrier.example.com/about",
...
assert.equal(normalized.offerCode, "PRO-AUG");
...
assert.equal(accepted.headers.get("X-Hermes-Document-Mode"), "review");
assert.match(accepted.headers.get("Content-Disposition") ?? "", /Hermes_TEST_Hermes_Carrier_LLC_Signed_Review_Packet\.pdf/);

const generatedPdf = Buffer.from(await accepted.arrayBuffer());
assert.ok(generatedPdf.length > 4_000);
...
const generatedText = generatedPdf.toString("latin1");
...
const serialized = JSON.stringify(serviceCalls[0].payload);
assert.match(serialized, /Document mode: review/);
assert.match(serialized, /Carrier-specific percentage: 8%/);
assert.match(serialized, /Website: https:\/\/carrier\.example\.com\/about/);
assert.match(serialized, /signed review packet/i);
assert.doesNotMatch(serialized, /"(?:password|passcode|pin|api[_-]?key|bank_account|routing_number)"\s*:/i);
assert.doesNotMatch(serialized, /business_address|preferred_lanes|load_boards|access_handoff_method|192\.0\.2\.55/i);
...
assert.ok(!sentMessages.some((message) => message.to === retiredRecipient));
...
assert.deepEqual(fallbackRecipients, ["officeus@hermeslogisticsus.com"]);
...
const forbidden = await onRequest({
  request: contractRequest(
    { ...validPayload, request_id: "contract_forbidden_12345", password: "must-never-be-accepted" },
...
for (const field of ["business_address", "equipment_types", "preferred_lanes", "load_boards", "access_handoff_method"]) {
  assert.equal(
    normalizeInput({ ...validPayload, [field]: field === "equipment_types" || field === "load_boards" ? ["Legacy"] : "Legacy" }),
    null,
    `${field} must be rejected from the minimized pre-signature payload.`,
  );
}
...
const custom = normalizeInput({
  ...
  custom_scope: "Hermes researches and negotiates within instructions; carrier retains final booking and all safety decisions.",
});
assert.ok(custom);
assert.equal(custom.percentage, "7.25% proposed - pending Hermes approval");
assert.equal(normalizeInput({ ...validPayload, typed_signature: "Someone Else" }), null);
assert.equal(normalizeInput({ ...validPayload, mc_number: "", usdot_number: "" }), null);
assert.equal(normalizeInput({ ...validPayload, company_website: "javascript:alert(1)" }), null);
...
const disallowedEnv = {
  ...
  CARRIER_CONTRACT_ALLOWED_PERCENTAGES: "6,9.5",
};
const reviewResponse = await onRequest({
  request: contractRequest(
    { ...validPayload, request_id: "contract_rate_not_allowed" },
    { "CF-Connecting-IP": "192.0.2.57" },
  ),
  env: disallowedEnv,
});
assert.equal(
  reviewResponse.headers.get("X-Hermes-Document-Mode"),
  "review",
  "A percentage absent from the production allowlist must fall back to review mode.",
);
...
assert.match(source, /master_document_hash_mismatch/);
assert.doesNotMatch(source, /CARRIER_CONTRACT_INTERNAL_RECIPIENTS/);
