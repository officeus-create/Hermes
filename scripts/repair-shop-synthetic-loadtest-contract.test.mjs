...
assert.match(syntheticSource, /INSERT INTO repair_shop_staff_schedule/);
assert.match(syntheticSource, /ON CONFLICT\(staff_id,day_of_week\) DO UPDATE SET/);
assert.match(syntheticSource, /\[0, 1, "07:00", "19:00"/);
assert.match(syntheticSource, /\[1, 1, "07:00", "19:00"/);
assert.match(syntheticSource, /start_time: "12:00", end_time: "12:30"/);
assert.match(syntheticSource, /\[6, 1, "07:00", "19:00"/);
...
assert.match(officeSeedSource, /const DEMO_OPEN_HOUR = 7/);
assert.match(officeSeedSource, /const DEMO_CLOSE_HOUR = 19/);
assert.match(officeSeedSource, /const DEMO_HISTORY_DAYS = 7/);
assert.match(officeSeedSource, /const DEMO_FORWARD_DAYS = 30/);
assert.match(officeSeedSource, /currentDemoWindow/);
assert.match(officeSeedSource, /end\.setUTCDate\(end\.getUTCDate\(\) \+ DEMO_FORWARD_DAYS - 1\)/);
assert.match(officeSeedSource, /officea\?/);
assert.match(officeSeedSource, /Array\.from\(\{ length: DEMO_CLOSE_HOUR - DEMO_OPEN_HOUR \}/);
assert.match(officeSeedSource, /parallel_booking_capacity,updated_at\)[\s\S]*VALUES \(\?, \?,1,1,1,10,\?\)/);
