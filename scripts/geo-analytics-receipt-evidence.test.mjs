...
const map = importGeoAnalyticsAttributionMap({ canonical_owner: carrierOwner, journey_path: carrierJourney, family: "carrier" });
const receipt = (eventName, overrides = {}) => ({ schema_version: geoAnalyticsReceiptVersion, reference_id: `receipt-${eventName}`, canonical_owner: carrierOwner, event_page_path: eventName === "commercial_cta_click" ? carrierOwner : carrierJourney, event_name: eventName, observed_at: "2026-08-18T18:00:00Z", receipt_state: "observed_once", synthetic: false, parameter_keys: geoAnalyticsRegistryContract[eventName].requiredParameterKeys, evidence_class: "platform_verified", supersedes_reference_id: null, ...overrides });
const ownership = importGeoAnalyticsOwnershipAttestation({ checked_at: "2026-08-18T17:00:00Z", ownership_state: "existing_property_confirmed", duplicate_tag_state: "none_observed", evidence_class: "platform_verified" });
...
const carrierEvents = ["commercial_cta_click", "carrier_intake_start", "carrier_intake_preview_ready", "carrier_handoff_ready", "carrier_delivery_confirmed"];
...
const missing = importGeoAnalyticsReceipt(receipt("carrier_intake_start", { reference_id: "receipt-missing-start", receipt_state: "not_observed" }));
assert.equal(missing.exactOnceVerified, false);
const duplicate = importGeoAnalyticsReceipt(receipt("carrier_intake_start", { reference_id: "receipt-duplicate-start", receipt_state: "observed_multiple" }));
assert.equal(duplicate.exactOnceVerified, false);
const unexpected = importGeoAnalyticsReceipt(receipt("commercial_cta_click", { reference_id: "receipt-cta-unexpected-key", observed_at: "2026-08-18T19:00:00Z", parameter_keys: [...geoAnalyticsRegistryContract.commercial_cta_click.requiredParameterKeys, "email"] }));
assert.deepEqual(unexpected.unexpectedParameterKeys, ["email"]);
assert.equal(unexpected.exactOnceVerified, false);
const incompleteParameters = importGeoAnalyticsReceipt(receipt("carrier_handoff_ready", { reference_id: "receipt-handoff-missing-param", parameter_keys: ["audience_type", "page_group", "service_group", "page_path", "preview_status"] }));
...
const synthetic = importGeoAnalyticsReceipt(receipt("carrier_intake_start", { reference_id: "receipt-synthetic-start", observed_at: "2026-08-18T19:00:00Z", synthetic: true }));
...
const older = receipt("carrier_intake_start", { reference_id: "receipt-start-old", observed_at: "2026-08-17T18:00:00Z" });
const newer = receipt("carrier_intake_start", { reference_id: "receipt-start-new", observed_at: "2026-08-18T18:00:00Z", supersedes_reference_id: "receipt-start-old" });
...
const seoMap = importGeoAnalyticsAttributionMap({ canonical_owner: "/services/seo/", journey_path: "/services/seo/", family: "seo" });
const seoHandoff = importGeoAnalyticsReceipt({ schema_version: geoAnalyticsReceiptVersion, reference_id: "receipt-seo-handoff", canonical_owner: "/services/seo/", event_page_path: "/services/seo/", event_name: "seo_handoff_ready", observed_at: "2026-08-18T18:00:00Z", receipt_state: "observed_once", synthetic: false, parameter_keys: geoAnalyticsRegistryContract.seo_handoff_ready.requiredParameterKeys, evidence_class: "platform_verified", supersedes_reference_id: null });
...
const ownerHandoffOnly = importGeoAnalyticsReceipt(receipt("carrier_intake_start", { reference_id: "receipt-owner-handoff-only", evidence_class: "owner_provided_handoff" }));
