...
const controlCenter = read("src/pages/services/hermes-connect/internal/ai-connect/index.astro");
...
assert.match(item, /action: payload\?\.action/, "owner task mutation must route through the bounded decision state machine");
assert.match(item, /requestedGate: payload\?\.approval_gate/, "owner approval must be scoped to the exact displayed gate");
...
assert.match(claim, /invalid_approval_continuation/, "runner claim must reject a gate without a persisted owner receipt");
...
assert.match(runner, /approved_continuation=approved_continuation/, "runner may resume an existing task branch only with the server approval receipt");
assert.match(runner, /Never infer approval from task text/, "task text must never manufacture an approval receipt");
...
assert.match(controlCenter, /data-approve/, "AI Connect must expose the scoped approval control after reload");
assert.match(controlCenter, /JSON\.stringify\(\{action:"approve",approval_gate:gate\}\)/, "AI Connect must send the exact persisted gate back to the owner API");
assert.match(controlCenter, /task\.status!=="needs_approval"\|\|!task\.approval_gate/, "AI Connect approval control must fail closed without a valid waiting task and gate");
