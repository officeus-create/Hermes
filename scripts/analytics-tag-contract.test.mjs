...
assert.equal((layout.match(loaderPattern) ?? []).length, 0); assert.match(layout, /<TrackingConsent\s+clarityEligible=\{clarityEligible\}\s+locale=\{locale\}\s*\/>/); assert.equal((consent.match(loaderPattern) ?? []).length, 1); assert.equal((connectConsent.match(loaderPattern) ?? []).length, 1);
...
for (const requiredAnalyticsCspHost of ["https://www.google.com", "https://*.clarity.ms", "https://c.bing.com"]) assert.ok(headers.includes(requiredAnalyticsCspHost));
...
assert.match(analytics, /Object\.entries\(payload\)\.filter\(\(\[, value\]\)\s*=>\s*value\s*!==\s*undefined\)/);
