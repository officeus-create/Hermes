...
const captured = [];
const successFetch = async (input, init) => {
...
const success = await handleRouteEstimate(
  { request: routeRequest(addressPayload), env: configuredEnv() },
  successFetch,
);
...
console.log("Route estimate endpoint checks passed: feature gate, same-origin, US/place validation, KV quota, minimal Google fields, safe success, provider failure, and zero secret leakage.");