...
assert.match(bookingSource, /const candidates = team\.configured[\s\S]*staffCoversWindow/);
assert.match(bookingSource, /for \(const candidate of candidates\)/);
assert.match(bookingSource, /technician_id = \?/);
assert.match(bookingSource, /NOT EXISTS[\s\S]*technician_id = \?/);
assert.match(bookingSource, /let inserted = 0/);
assert.match(bookingSource, /inserted = Number\(results\?\.\[0\]\?\.meta\?\.changes/);
assert.match(bookingSource, /if \(inserted === 1\) \{[\s\S]*assignedStaff = candidate;[\s\S]*break;/);
