...
assert.deepEqual(Object.keys(LOAD_BOARD_ADAPTERS).sort(), ["central_dispatch", "dat", "sanitized_csv", "ship_cars", "super_dispatch", "truckstop"]);
...
const providerRead = evaluateLoadBoardAdapterAccess({ providerId: "central_dispatch", environment: "sandbox", operation: "read_opportunities", ownerApproved: true, commercialReviewPassed: true, dataRightsReviewPassed: true });
...
const write = evaluateLoadBoardAdapterAccess({ providerId: "truckstop", environment: "production", operation: "write_or_book" });
assert.equal(write.allowed, false);
assert.match(write.reason, /writes and bookings are disabled/i);

const publicExport = evaluateLoadBoardAdapterAccess({ providerId: "dat", environment: "preview", operation: "public_export" });
assert.equal(publicExport.allowed, false);
assert.match(publicExport.reason, /public marketplace export is disabled/i);

const webhook = evaluateLoadBoardAdapterAccess({ providerId: "super_dispatch", environment: "sandbox", operation: "receive_webhook" });
assert.equal(webhook.allowed, false);
assert.match(webhook.reason, /webhooks are not configured/i);

const incompleteCsv = evaluateLoadBoardAdapterAccess({ providerId: "sanitized_csv", environment: "preview", operation: "import_preview", ownerApproved: true, sourceApproved: true, privacyReviewPassed: false, dataRightsReviewPassed: true });
assert.equal(incompleteCsv.allowed, false);
assert.match(incompleteCsv.reason, /privacy review/i);

const approvedCsvPreview = evaluateLoadBoardAdapterAccess({ providerId: "sanitized_csv", environment: "preview", operation: "import_preview", ownerApproved: true, sourceApproved: true, privacyReviewPassed: true, dataRightsReviewPassed: true });
...
const csvProduction = evaluateLoadBoardAdapterAccess({ providerId: "sanitized_csv", environment: "production", operation: "import_preview", ownerApproved: true, sourceApproved: true, privacyReviewPassed: true, dataRightsReviewPassed: true });
