...
const blocks = [...homepage.matchAll(/<script[^>]+type=["']application\/ld\+json["'][^>]*>([\s\S]*?)<\/script>/gi)]
  .map((match) => JSON.parse(match[1]));
const entities = blocks.flatMap((block) => Array.isArray(block) ? block : [block]);
...
console.log("Entity schema audit passed: stable Organization, WebSite, logo, contact, site-name, language, and publisher signals.");
