#!/usr/bin/env python3
"""Outbound-only local runner for Hermes Internal AI Assistant tasks.

Required local environment:
  HERMES_INTERNAL_AI_API=https://hermeslogisticsus.com
  HERMES_INTERNAL_AI_RUNNER_TOKEN=<scoped secret stored locally only>
...
POLL_SECONDS = max(5, int(os.environ.get("HERMES_INTERNAL_AI_POLL_SECONDS", "10")))
STATE_POLL_SECONDS = max(5, int(os.environ.get("HERMES_INTERNAL_AI_STATE_POLL_SECONDS", "10")))
EVENT_FLUSH_SECONDS = 2.0
MAX_EVENT_CHARS = 3500
MAX_SUMMARY_CHARS = 18000
RUNNER_TIMEOUT_SECONDS = max(60, int(os.environ.get("HERMES_INTERNAL_AI_TASK_TIMEOUT_SECONDS", "3600")))

SCRIPT = Path(__file__).resolve()
REPO = SCRIPT.parents[2]
CODEX_HERMES = REPO / "scripts" / "ai" / "codex-hermes"
API_BASE = os.environ.get("HERMES_INTERNAL_AI_API", "").rstrip("/")
TOKEN = os.environ.get("HERMES_INTERNAL_AI_RUNNER_TOKEN", "")
# Verified against the installed official `codex exec --help`: these options
# belong after `exec`. This keeps the workspace sandbox enabled while routing
# routine approvals through Codex's safe automatic review.
CODEX_AUTONOMOUS_ARGS = ("--sandbox", "workspace-write", "--approve-for-me")
...
APPROVAL_MARKER = re.compile(r"^HERMES_INTERNAL_APPROVAL_GATE\s*=\s*([a-z_]+)\s*$", re.MULTILINE)


def die(message: str) -> None:
    print(f"[hermes-internal-ai-runner] ERROR: {message}", file=sys.stderr)
    raise SystemExit(1)
...
def restore_main_if_safe(task_branch: str | None = None, starting_sha: str | None = None) -> None:
    ...
    if tracked_worktree_changes():
        print("[hermes-internal-ai-runner] tracked task changes remain; leaving checkout untouched for review", file=sys.stderr)
        return
...
If and only if you reach a consequential owner gate, do not perform that action.
State the evidence and print exactly one line on its own:
...
def request_json(path: str, *, method: str = "GET", payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "HermesInternalAiRunner/1.0",
        },
    )
...
def task_state(task_id: str) -> dict[str, Any] | None:
    ...
    response = request_json(f"/api/internal-ai/runner/task?{query}")
...
def claim() -> dict[str, Any] | None:
    ...
    response = request_json("/api/internal-ai/runner/claim", method="POST", payload=payload)
...
def complete(task_id: str, *, status: str, output: str, return_code: int | None) -> None:
    ...
    request_json(
        "/api/internal-ai/runner/complete",
        method="POST",
        payload={
            "task_id": task_id,
            "status": status,
...
def execute_task(task: dict[str, Any]) -> None:
    ...
    try:
        while process.poll() is None:
            ...
            if now - last_state_check >= STATE_POLL_SECONDS:
                ...
                try:
                    ...
                    print(f"[hermes-internal-ai-runner] cancel/heartbeat poll warning: {sanitize(str(exc), 300)}", file=sys.stderr)
            ...
            if buffered and (time.monotonic() - last_flush >= EVENT_FLUSH_SECONDS or sum(map(len, buffered)) >= MAX_EVENT_CHARS):
                ...
                if chunk:
                    try:
                        ...
                        print(f"[hermes-internal-ai-runner] event upload warning: {sanitize(str(exc), 300)}", file=sys.stderr)
        ...
        approval_match = APPROVAL_MARKER.search(summary_tail)
        approval_gate = approval_match.group(1) if approval_match else None
        if approval_gate not in APPROVAL_GATES:
            approval_gate = None
...
def main() -> None:
    if not API_BASE.startswith("https://"):
        die("HERMES_INTERNAL_AI_API must be an https:// origin.")
    if not TOKEN:
        die("HERMES_INTERNAL_AI_RUNNER_TOKEN is required and must stay local.")
    if not (REPO / "AGENTS.md").exists():
        die(f"Hermes repository not found at {REPO}")

    print(f"[hermes-internal-ai-runner] outbound poller started for {API_BASE}; repo={REPO}")
    while True:
        try:
            task = claim()
            if task:
            ...
            print("\n[hermes-internal-ai-runner] stopped by user")
            return
        except urllib.error.HTTPError as exc:
            detail = exc.read().decode("utf-8", errors="replace")[:500]
            print(f"[hermes-internal-ai-runner] HTTP {exc.code}: {sanitize(detail, 500)}", file=sys.stderr)
            time.sleep(POLL_SECONDS)
        except Exception as exc:
            print(f"[hermes-internal-ai-runner] warning: {sanitize(str(exc), 500)}", file=sys.stderr)
            time.sleep(POLL_SECONDS)
