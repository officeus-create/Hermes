...
assert.equal((hubSource.match(/status: "LIVE PRODUCT"/g) ?? []).length, 1);
assert.match(hubSource, /PRODUCT PREPARATION/);
assert.match(hubSource, /PILOT PREPARATION/);
assert.match(hubSource, /\/services\/hermes-connect\/academy\//);
assert.match(hubSource, /\/services\/hermes-connect\/beauty-salons\//);
assert.match(hubSource, /Open preparation/);
