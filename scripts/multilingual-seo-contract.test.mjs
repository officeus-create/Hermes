...
const supportedHreflang = new Set(["en", "uk", "ru", "es", "fr", "it", "x-default"]);
const errors = [];
...
const tagText = (html, name) => decode(
...
const localeForRoute = (route) => {
  ...
  return localePrefixes.get(first) ?? (route === "/" ? "en" : null);
};
const robotsValue = (html) => metaTags(html)
  .filter((tag) => getAttr(tag, "name").toLowerCase() === "robots")
  .map((tag) => getAttr(tag, "content").toLowerCase())
  .join(",");
const canonicalUrls = (html) => linkTags(html)
  .filter((tag) => getAttr(tag, "rel").toLowerCase().split(/\s+/).includes("canonical"))
  .map((tag) => getAttr(tag, "href"));
const alternates = (html) => new Map(
  linkTags(html)
...
const sitemapLocs = new Set(
  [...sitemapXml.matchAll(/<loc>\s*([^<]+)\s*<\/loc>/gi)].map((match) => decode(match[1])),
);
...
for (const file of localizedFiles) {
  ...
  if (lang !== locale) errors.push(`${route}: expected html lang=${locale}, received ${lang || "missing"}`);

  const canonicals = canonicalUrls(html);
  if (canonicals.length !== 1 || canonicals[0] !== canonical) {
    errors.push(`${route}: self-canonical expected ${canonical}, received ${canonicals.join(", ") || "missing"}`);
  }
  ...
  if (!noindex && visibleText(html).length < 300) {
    errors.push(`${route}: indexable localized page is unexpectedly thin (<300 visible characters)`);
  }
  ...
  if (!noindex && !sitemapLocs.has(canonical)) {
    errors.push(`${route}: indexable localized canonical is missing from controlled sitemap inventory`);
  }
  if (noindex && sitemapLocs.has(canonical)) {
    errors.push(`${route}: noindex localized URL must not appear in sitemap inventory`);
  }

  const pageAlternates = alternates(html);
  for (const [alternateLang, href] of pageAlternates) {
    if (!supportedHreflang.has(alternateLang)) errors.push(`${route}: unsupported hreflang=${alternateLang}`);
    ...
    try {
      ...
      errors.push(`${route}: invalid hreflang URL for ${alternateLang}: ${href}`);
      continue;
    }
    if (target.origin !== origin) continue;
    if (target.search || target.hash) {
      errors.push(`${route}: hreflang ${alternateLang} must not contain query/hash: ${target.href}`);
    }
    ...
    if (!targetHtml) {
      errors.push(`${route}: hreflang ${alternateLang} points to missing built page ${targetRoute}`);
      continue;
    }
    const targetCanonical = canonicalUrls(targetHtml);
    if (targetCanonical.length !== 1 || targetCanonical[0] !== `${origin}${targetRoute}`) {
      errors.push(`${route}: hreflang target ${targetRoute} is not self-canonical`);
    }
    const reciprocal = alternates(targetHtml).get(locale);
    if (reciprocal !== canonical) {
      errors.push(`${route}: hreflang ${alternateLang} target ${targetRoute} does not reciprocate ${locale} -> ${canonical}`);
    }

    const sourceDepth = route.split("/").filter(Boolean).length;
    const targetDepth = targetRoute.split("/").filter(Boolean).length;
    if (sourceDepth > 1 && targetDepth <= 1) {
      errors.push(`${route}: deep localized page must not use generic locale/homepage fallback as hreflang ${alternateLang} -> ${targetRoute}`);
    }
  }

  if (pageAlternates.size > 0 && pageAlternates.get(locale) !== canonical) {
    errors.push(`${route}: page with hreflang cluster must include self-reference ${locale} -> ${canonical}`);
  }
}
...
for (const [route, html] of htmlByRoute) {
  ...
  for (const [lang, href] of pageAlternates) {
    if (!supportedHreflang.has(lang) && lang !== "ua") errors.push(`${route}: unsupported hreflang=${lang}`);
    if (!href) errors.push(`${route}: empty hreflang href for ${lang}`);
  }
}

if (errors.length) {
  throw new Error(`Multilingual SEO contract failed with ${errors.length} error(s):\n${errors.map((error) => `- ${error}`).join("\n")}`);
}

console.log(
  `Multilingual SEO contract passed: ${localizedFiles.length} localized HTML pages audited across RU/UK/ES/FR/IT for lang, self-canonical, indexability/sitemap ownership, metadata, H1, thin-content floor and reciprocal hreflang integrity.`,
);
