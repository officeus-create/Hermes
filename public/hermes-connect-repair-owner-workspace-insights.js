(() => {
  ...
  const strings = {
    en: { scheduleKicker:"BOOKING SCHEDULE · LIVE", scheduleTitle:"Upcoming service", scheduleEmpty:"No confirmed or in-progress bookings are active right now.", customersKicker:"RECENT CUSTOMERS · LIVE", customersTitle:"Customer activity", customersEmpty:"Customer activity will appear after the first real booking.", crm:"Open customer CRM ↗", opportunityKicker:"RECOVERABLE OPPORTUNITY · LIVE", opportunityTitle:"What can move next", cancelled:(n)=>`${n} cancelled booking${n===1?"":"s"} can be reviewed for a customer follow-up.`, completed:(n)=>`${n} completed visit${n===1?"":"s"} can now feed the next-appointment follow-up loop.`, active:(n)=>`${n} active booking${n===1?"":"s"} need service completion before the next follow-up signal.`, first:"Your public booking link is ready. The next proof is the first real customer booking.", setup:"Finish profile, services and public booking readiness before Hermes claims a growth opportunity.", capacity:"Open-capacity recommendations stay hidden until real availability and appointment data can support them.", booking:"Review bookings", customer:"Customer" },
    ru: { scheduleKicker:"ГРАФИК ЗАПИСЕЙ · LIVE", scheduleTitle:"Ближайший сервис", scheduleEmpty:"Сейчас нет активных записей со статусом «подтверждено» или «в работе».", customersKicker:"ПОСЛЕДНИЕ КЛИЕНТЫ · LIVE", customersTitle:"Активность клиентов", customersEmpty:"Активность появится после первой реальной записи.", crm:"Открыть CRM клиентов ↗", opportunityKicker:"ВОЗВРАЩАЕМАЯ ВОЗМОЖНОСТЬ · LIVE", opportunityTitle:"Что можно улучшить следующим", cancelled:(n)=>`Отменённых записей: ${n}. Их можно проверить для follow-up клиента.`, completed:(n)=>`Завершённых визитов: ${n}. Следующий реальный шаг — follow-up и следующая запись клиента.`, active:(n)=>`Активных записей: ${n}. Сначала завершите обслуживание, затем Hermes покажет следующий follow-up сигнал.`, first:"Публичная ссылка готова. Следующее доказательство — первая реальная запись клиента.", setup:"Сначала завершите профиль, услуги и публичную запись — до этого Hermes не будет выдумывать возможность роста.", capacity:"Рекомендации по свободной мощности скрыты, пока их нельзя подтвердить реальными availability и appointments.", booking:"Проверить записи", customer:"Клиент" },
  ...
  const render = () => {
    ...
    if (schedule) {
      schedule.innerHTML = activeRows.length ? activeRows.slice(0,4).map((row) => `<div><span>${escapeHtml(row.when)}</span><p><strong>${escapeHtml(row.service)}</strong><small>${escapeHtml(row.customer)}${row.vehicle ? ` · ${escapeHtml(row.vehicle)}` : ""}</small></p></div>`).join("") : `<p>${t.scheduleEmpty}</p>`;
    }
    ...
    const opportunity = cancelled > 0 ? t.cancelled(cancelled)
      : completed > 0 ? t.completed(completed)
      : active > 0 ? t.active(active)
      : publicReady ? t.first
      : t.setup;
