...
const ownerRules = [
  ...
  {
    id: "car_hauling_dispatch",
    test: (query) => /\b(car\s+hauler|car\s+hauling|auto\s+transport)\b.*\bdispatch(?:er|ing|\s+service)?\b|\bdispatch(?:er|ing|\s+service)?\b.*\b(car\s+hauler|car\s+hauling|auto\s+transport)\b/i.test(query),
    owner: "/logistics/car-hauling-dispatch/",
    commercialWeight: 3,
    supported: true,
    label: "Car-hauling dispatch service",
  },
  {
    id: "car_hauler_market",
    test: (query) => /\b(car\s+hauler|car\s+hauling|auto\s+transport)\b.*\b(buffalo|rochester|western\s+new\s+york|upstate\s+new\s+york|new\s+york|georgia|atlanta|north\s+carolina|south\s+carolina)\b|\b(buffalo|rochester|western\s+new\s+york|upstate\s+new\s+york|new\s+york|georgia|atlanta|north\s+carolina|south\s+carolina)\b.*\b(car\s+hauler|car\s+hauling|auto\s+transport)\b/i.test(query),
    owner: "/logistics/car-hauler-loads/",
    commercialWeight: 2,
    supported: true,
    label: "Car-hauler operating-market research",
  },
  {
    id: "car_hauler_loads",
    test: (query) => /\b(?:\d+\s+)?car\s+hauler\s+loads?\b(?!\s+board)|\bcar\s+hauling\s+loads?\b(?!\s+board)|\bauto\s+transport\s+loads?\b(?!\s+board)|\bcar\s+carrier\s+loads?\b(?!\s+board)|\bhow\s+to\s+find\s+car\s+hauling\s+loads?\b/i.test(query),
    owner: "/load-board/equipment/car-hauler/",
    commercialWeight: 2,
    supported: true,
    label: "Car-hauler loads and equipment-fit search",
  },
  {
    id: "load_board",
    test: (query) => /\bload\s+boards?\b|\b(car\s+hauler|car\s+hauling|auto\s+transport|car\s+transport|car\s+carrier)\s+load\s+boards?\b/i.test(query),
    owner: "/load-board/",
    commercialWeight: 3,
    supported: true,
