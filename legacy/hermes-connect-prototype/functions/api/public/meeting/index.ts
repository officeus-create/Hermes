...
export async function onRequestPost({ request, env }: { request: Request; env: Env }) {
  ...
  const meetingTopic = cleanText(body.meetingTopic, 300);
  ...
  const isReturningClient = await env.DB
    .prepare("SELECT 1 FROM bookings WHERE specialist_id = ? AND client_email = ? AND status = 'confirmed' LIMIT 1")
    .bind(specialistId, clientEmail)
    .first();
  ...
  await env.DB.prepare(
    ...
    .bind(id, specialistId, staffId || null, slot, status, clientName, clientEmail, contactMethod, contactHandle, meetingTopic, accessToken, createdAt)
    .run();
