...
test("Logistics commercial directory remains usable at 390px", async ({ page }) => {
  ...
  const mobile = await page.evaluate(() => ({
    overflow: document.documentElement.scrollWidth > document.documentElement.clientWidth,
    cardRadius: getComputedStyle(document.querySelector<HTMLElement>(".logistics-commercial-grid article")!).borderRadius,
    directoryRadius: getComputedStyle(document.querySelector<HTMLElement>(".logistics-path-directory")!).borderRadius,
  }));
