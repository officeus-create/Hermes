...
for (const route of trustRoutes) {
  test(`${route} uses the shared Pearl trust system`, async ({ page }) => {
    ...
    const visual = await page.evaluate(() => {
      ...
      if (!hero || !card || !contact || !action) return null;
      return {
...
test("Trust family remains usable at 390px", async ({ page }) => {
  ...
  const mobile = await page.evaluate(() => {
    ...
    return {overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth,contactRadius:getComputedStyle(contact).borderRadius,actionWidth:action.getBoundingClientRect().width,contactWidth:contact.getBoundingClientRect().width};
  });
