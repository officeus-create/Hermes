...
const openLoadForm = async (page: Page) => {
  ...
  await expect(page.locator("[data-load-board-form] [data-route-estimate-panel]")).toHaveCount(1);
};
...
test.describe("Route estimate boundary", () => {
  ...
  test("uses a clear offline fallback and does not claim the request was sent", async ({ page }) => {
    ...
    await expect(page.locator("[data-route-estimate-result]")).toBeHidden();
    await expect(page.getByRole("link", { name: "Logistics Sales contact options" })).toHaveAttribute("href", "/contacts/");
  });

  test("rejects a cross-origin endpoint before any request", async ({ page }) => {
    let routeRequests = 0;
    ...
    await page.locator("[data-route-estimate-panel]").evaluate((panel: Element) => {
      if (!(panel instanceof HTMLElement)) throw new Error("route panel missing");
      panel.dataset.routeEstimateMode = "live";
      panel.dataset.routeEstimateEndpoint = "https://attacker.example/collect";
    });
    await page.getByRole("button", { name: "Estimate route" }).click();
    await expect(page.locator("[data-route-estimate-status]")).toContainText("unavailable");
    expect(routeRequests).toBe(0);
  });
});
