...
test("Load Board starts with two plain-language entry choices and keeps the live/preview boundary explicit", async ({ page }) => {
  ...
  await expect(page.getByText(/Live \+ preview split:/i)).toBeVisible();
  ...
  await expect(page.getByText(/Preview rows are separate, clearly marked, and non-bookable/i)).toBeVisible();
});
