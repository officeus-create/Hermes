...
async function mockWorkspace(page: any) {
  ...
  await page.route("**/api/repair-shop/bookings", (route: any) => route.fulfill(ok({ success: true, bookings: [{ id: "booking-xss", service_name: maliciousService, duration_minutes: 30, appointment_date: "2026-08-19", start_time: "10:00", end_time: "10:30", status: "confirmed", client_name: maliciousCustomer, client_email: "safe@example.com", client_phone: "+14145550111", vehicle: { year: 2022, make: "Toyota", model: "Camry", mileage: 1000, vin: null }, history: [] }] })));
...
test("active booking schedule and customer insights never reinterpret stored text as markup", async ({ page }) => {
  ...
  await expect(page.locator("[data-hc-live-opportunity-copy]")).toContainText("Активных записей: 1");
