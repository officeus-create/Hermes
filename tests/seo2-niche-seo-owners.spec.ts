...
for (const scenario of [
  ...
  test(`${scenario.path} owns its niche intent and keeps the approved SEO funnel context`, async ({ page }) => {
    ...
    await expect(page.locator(".digital-breadcrumb").getByRole("link", { name: "Marketing" })).toHaveAttribute("href", "/paths/marketing/");
    ...
    for (const text of scenario.evidence) await expect(page.getByText(text, { exact: true })).toBeVisible();
  });
}

test("Local SEO keeps the separate current-main local SEO context", async ({ page }) => {
  await page.goto("/services/local-seo/");
  await expect(page.locator(".digital-service-actions").getByRole("link", { name: "Start a local SEO review" })).toHaveAttribute("href", "/paths/marketing/?service=local_seo#contact");
  await expect(page.getByRole("link", { name: "Prepare the local SEO request" })).toHaveAttribute("href", "/paths/marketing/?service=local_seo#contact");
  await expect(page.locator('select[name="path"]')).toHaveValue("ProgressoPro");
});