...
const onboardingRoute = "/logistics/carrier-onboarding/?plan=pro&rate=8&rep=TEST%20Assistant%20107&offer=PRO-AUG";
...
async function completeCarrierPacket(page: Page) {
  ...
  await expect(page.locator('input[name="sales_contact"]')).toHaveValue("TEST Assistant 107");
  await expect(page.locator('input[name="offer_code"]')).toHaveValue("PRO-AUG");
...
test("short carrier link preserves only safe commercial context", async ({ page }) => {
  ...
  await expect(primary).toHaveAttribute("href", /\/logistics\/carrier-onboarding\/\?plan=pro&rate=8&rep=Assistant(?:%20|\+)107&offer=PRO-AUG/);
  const href = await primary.getAttribute("href");
  expect(href).not.toMatch(/carrier_name|mc=|123456/);
...
test("custom plan requires scope and remains a review proposal", async ({ page }) => {
  await page.goto("/logistics/carrier-onboarding/?plan=custom&rate=7.25");
  await expect(page.locator('input[name="selected_plan"][value="custom"]')).toBeChecked();
  await expect(page.locator('input[name="service_percentage"]')).toHaveValue("7.25");
