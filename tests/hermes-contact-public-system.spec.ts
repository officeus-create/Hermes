...
for (const route of routes) {
  test(`${route} uses the shared Hermes contact primitive`, async ({ page }) => {
    ...
    const visual = await page.evaluate(() => {
      ...
      return {sectionBackground:getComputedStyle(section).backgroundColor,sectionColor:getComputedStyle(section).color,routesBackground:getComputedStyle(routesEl).backgroundColor,routesRadius:getComputedStyle(routesEl).borderRadius,formBackground:getComputedStyle(form).backgroundColor,formRadius:getComputedStyle(form).borderRadius,inputBackground:getComputedStyle(input).backgroundColor,inputRadius:getComputedStyle(input).borderRadius,primaryBackground:getComputedStyle(primary).backgroundColor,primaryColor:getComputedStyle(primary).color,primaryRadius:getComputedStyle(primary).borderRadius,overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth};
    });
    const expectedFormRadius=(page.viewportSize()?.width??1440)<=900?"22px":"30px";
...
test("shared contact primitive remains usable at 390px", async ({ page }) => {
  ...
  await expect(page.locator(".contact-form")).toBeVisible();
  const mobile=await page.evaluate(()=>{const form=document.querySelector<HTMLElement>(".contact-form");const primary=document.querySelector<HTMLElement>(".contact-form .button-primary");if(!form||!primary)return null;return{overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth,formRadius:getComputedStyle(form).borderRadius,primaryWidth:primary.getBoundingClientRect().width,formWidth:form.getBoundingClientRect().width};});
