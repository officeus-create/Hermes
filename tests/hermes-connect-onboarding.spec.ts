...
test.describe("Hermes Connect Onboarding Business Type Selector", () => {
  ...
  test("Deep-linking with ?business_type= parameter bypasses onboarding and loads correct vertical", async ({ page }) => {
    // 1. Visit workspace with business_type parameter
    await page.goto(`${workspace}?business_type=agency`);
  ...
  test("Deep-linking with workspace URL parameters (tool/module/tab) bypasses onboarding", async ({ page }) => {
    // 1. Visit workspace with a standard parameter (e.g., tool)
    await page.goto(`${workspace}?tool=onboarding`);
  ...
  test("Mobile first visit shows onboarding, allows selection and launches custom UI", async ({ page }, testInfo) => {
    ...
    // 4. Launch workspace
    await page.locator('[data-launch-workspace-btn]').click({ force: true });
