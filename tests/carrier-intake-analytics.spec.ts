...
const readEvents = (eventName: string) =>
  window.dataLayer?.filter((item: Record<string, unknown>) => item.event === eventName) ?? [];
...
test("carrier intake emits one start and one privacy-safe preview event", async ({ page }) => {
  ...
  await expect.poll(async () => page.evaluate(() => readEvents("carrier_intake_start").length)).toBe(1);

  await form.getByRole("button", { name: /Review access request/ }).click();
  await expect(page.locator("[data-vehicle-result]")).toBeVisible();
  await expect.poll(async () => page.evaluate(() => readEvents("carrier_intake_preview_ready").length)).toBe(1);

  const analyticsEvents = await page.evaluate(() => ({
    starts: readEvents("carrier_intake_start"),
    previews: readEvents("carrier_intake_preview_ready"),
  }));

  expect(analyticsEvents.starts[0]).toMatchObject({
  ...
  expect(analyticsEvents.previews[0]).toMatchObject({
  ...
  const serialized = JSON.stringify(analyticsEvents);
...
test("invalid carrier intake does not emit a preview-ready event", async ({ page }) => {
  ...
  await expect.poll(async () => page.evaluate(() => readEvents("carrier_intake_preview_ready").length)).toBe(0);
});
