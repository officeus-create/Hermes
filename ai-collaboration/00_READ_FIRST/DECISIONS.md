...
| DEC-007 | 2026-08-23 | AI collaboration / governance | Supersede the prior Codex=implementation / Claude=review-only split. Codex and Claude are now co-primary peer execution agents with equal baseline authority: both may analyze, write and modify code, run tests, create branches/PRs, update documentation and handoffs, and make bounded technical decisions independently when supported by current code, tests, an existing owner directive, or production evidence. Specialization (e.g. Codex on implementation throughput, Claude on independent QA/security/evidence review) informs task assignment per-slice but no longer implies a fixed hierarchy. Per-slice task ownership (one primary executor, one reviewer, named branch/write-scope) still applies per the anti-loop rule in `AI_COLLABORATION_PROTOCOL.md`; peer status does not permit two agents to edit the same files unco­ordinated. Material/irreversible/destructive/production-critical/billing/credential/security-ownership decisions remain Owner-gated regardless of this change. | Approved | Vladimir / Hermes | Owner directive, this thread, 2026-08-23; supersedes the routing note in `CURRENT_STATE.md` | Both agents complete scoped work without artificial escalation to the other; task-ownership records show the executor/reviewer role rotating by task rather than by fixed identity; no unco­ordinated concurrent edits to the same files. |
