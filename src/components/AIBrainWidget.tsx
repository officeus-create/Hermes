...
import { Bot, Send, Sparkles, CheckCircle2, ChevronRight, X, MessageSquare, Zap } from 'lucide-react';
    ...
    <div className="fixed bottom-6 right-6 z-50">
      {!isOpen ? (
        <button
          ...
          className="btn-primary rounded-full p-4 shadow-2xl hover:scale-105 transition-all group flex items-center gap-3 bg-gradient-to-r from-indigo-600 via-purple-600 to-cyan-500"
        >
          ...
          <span className="font-bold text-sm hidden sm:inline">Front-Door AI Brain</span>
        </button>
      ) : (
        <div className="glass-panel w-[380px] sm:w-[420px] h-[540px] flex flex-col shadow-2xl border border-indigo-500/30 overflow-hidden animate-in fade-in slide-in-from-bottom-5 duration-200">
          {/* Header */}
          <div className="p-4 bg-slate-900/90 border-b border-white/10 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-bold text-sm text-white font-heading">Front-Door AI Brain</h4>
                <p className="text-[11px] text-emerald-400 flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                  Intent Routing & Cross-Sell Active
                </p>
              </div>
            </div>
            <button
          ...
          <div className="p-2 bg-slate-900/80 border-t border-white/5 flex items-center gap-1.5 overflow-x-auto text-[11px]">
            <button
              onClick={() => onSendMessage('Хочу записать машину на ТО')}
              className="px-2.5 py-1 rounded-full bg-white/5 hover:bg-white/10 text-slate-300 whitespace-nowrap border border-white/5"
            >
              🚗 Запись авто
            </button>
            <button
          ...
          {/* Input Bar */}
          <form onSubmit={handleSubmit} className="p-3 bg-slate-900 border-t border-white/10 flex items-center gap-2">
            <input
              type="text"
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder="Задайте вопрос AI Brain..."
