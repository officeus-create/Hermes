...
const latestReceipts = (receipts: GeoAnalyticsReceipt[]) => { const superseded = new Set(receipts.map((item) => item.supersedesReferenceId).filter((item): item is string => Boolean(item))); const map = new Map<string, GeoAnalyticsReceipt>(); for (const receipt of receipts.filter((item) => !superseded.has(item.referenceId))) { const key = `${receipt.canonicalOwner}|${receipt.eventPagePath}|${receipt.eventName}`; const current = map.get(key); if (!current || Date.parse(receipt.observedAt) > Date.parse(current.observedAt)) map.set(key, receipt); } return [...map.values()]; };
export const buildGeoAnalyticsReceiptHealth = (receipts: GeoAnalyticsReceipt[]) => { const latest = latestReceipts(receipts); const real = latest.filter((item) => !item.synthetic); return { latestRealReceipts: real, exactOnce: real.filter((item) => item.exactOnceVerified), missingReceipt: real.filter((item) => item.receiptState === "not_observed"), duplicateReceipt: real.filter((item) => item.receiptState === "observed_multiple"), parameterPrivacyIssues: real.filter((item) => item.unexpectedParameterKeys.length > 0), parameterCompletenessIssues: real.filter((item) => item.missingRequiredParameterKeys.length > 0), syntheticExcludedCount: latest.filter((item) => item.synthetic).length }; };
export const buildGeoAnalyticsFreshness = (receipts: GeoAnalyticsReceipt[], asOf: string, freshnessDays: number) => { const asOfMs = Date.parse(timestamp(asOf, "asOf")); if (!Number.isInteger(freshnessDays) || freshnessDays < 1 || freshnessDays > 90) throw new Error("freshnessDays must be 1-90"); return latestReceipts(receipts).map((receipt) => { const ageDays = Math.floor((asOfMs - Date.parse(receipt.observedAt)) / 86_400_000); if (ageDays < 0) throw new Error(`Analytics receipt ${receipt.referenceId} occurs after asOf`); const freshnessState: GeoAnalyticsFreshnessState = ageDays <= freshnessDays ? "current" : "stale"; return { ...receipt, ageDays, freshnessDays, freshnessState }; }); };
export const buildGeoAnalyticsOwnerCompleteness = (maps: GeoAnalyticsAttributionMapInput[], receipts: GeoAnalyticsReceipt[]) => { const latest = latestReceipts(receipts).filter((item) => !item.synthetic); return maps.map((map) => { const states = familyEvents[map.family].map((eventName) => { const expectedPage = eventName === "commercial_cta_click" ? map.canonical_owner : map.journey_path; const receipt = latest.find((item) => item.canonicalOwner === map.canonical_owner && item.eventName === eventName && item.eventPagePath === expectedPage); return { eventName, expectedPage, state: receipt?.exactOnceVerified ? "exact_once_verified" as const : receipt?.receiptState === "observed_multiple" ? "duplicate_receipt" as const : receipt?.receiptState === "not_observed" ? "not_observed" as const : receipt ? "receipt_not_verified" as const : "missing_evidence" as const }; }); return { canonicalOwner: map.canonical_owner, journeyPath: map.journey_path, family: map.family, states, complete: states.every((item) => item.state === "exact_once_verified"), missingOrInvalidEvents: states.filter((item) => item.state !== "exact_once_verified").map((item) => item.eventName) }; }); };
export const reconcileGeoAnalyticsHandoffDelivery = (map: GeoAnalyticsAttributionMapInput, receipts: GeoAnalyticsReceipt[]) => { const expectedDelivery = deliveryEvent[map.family] ?? null; const handoffName: GeoCanonicalAnalyticsEvent = map.family === "carrier" ? "carrier_handoff_ready" : map.family === "vehicle_transport" ? "vehicle_transport_handoff_ready" : map.family === "seo" ? "seo_handoff_ready" : "website_handoff_ready"; const latest = latestReceipts(receipts).filter((item) => !item.synthetic && item.canonicalOwner === map.canonical_owner); const handoff = latest.find((item) => item.eventName === handoffName && item.eventPagePath === map.journey_path) ?? null; if (!expectedDelivery) return { canonicalOwner: map.canonical_owner, family: map.family, handoffExactOnce: Boolean(handoff?.exactOnceVerified), deliveryEvent: null, deliveryState: "delivery_event_not_established_in_canonical_registry" as const }; const delivery = latest.find((item) => item.eventName === expectedDelivery && item.eventPagePath === map.journey_path) ?? null; return { canonicalOwner: map.canonical_owner, family: map.family, handoffExactOnce: Boolean(handoff?.exactOnceVerified), deliveryEvent: expectedDelivery, deliveryState: delivery?.exactOnceVerified ? "delivery_exact_once_verified" as const : delivery ? "delivery_receipt_not_verified" as const : "delivery_evidence_missing" as const }; };
