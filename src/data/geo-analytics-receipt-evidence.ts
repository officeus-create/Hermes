...
const latestReceipts = (receipts: GeoAnalyticsReceipt[]) => { const superseded = new Set(receipts.map((item) => item.supersedesReferenceId).filter((item): item is string => Boolean(item))); const map = new Map<string, GeoAnalyticsReceipt>(); for (const receipt of receipts.filter((item) => !superseded.has(item.referenceId))) { const evidenceKind = receipt.synthetic ? "synthetic" : "real"; const key = `${receipt.canonicalOwner}|${receipt.eventPagePath}|${receipt.eventName}|${evidenceKind}`; const current = map.get(key); if (!current || Date.parse(receipt.observedAt) > Date.parse(current.observedAt)) map.set(key, receipt); } return [...map.values()]; };
