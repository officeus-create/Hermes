...
export const academyPublicPathOverrides: Partial<PathDetail> = {
  ...
  overview: "Hermes Business Academy currently presents two public program paths: U.S. Logistics Operations and Marketing. The former COO / Operational Director Program, including the shorter COO / Operational Director wording, Operating Career System, Executive, and ‘Three programs. Different responsibilities.’ language, is retired and is not a current public program. The Practice environment remains part of the two approved paths. Program dates, scope, and prices are published before enrollment. Ask about the right Academy path. Training and practice do not guarantee employment. The former Marketing lab wording ‘Learn AI automation by building one useful assistant.’, ‘Fitness AI Telegram Assistant’, ‘Learning preview only’, and ‘Ask about learning AI automation’ refers only to an optional exercise inside Marketing, not a separate program.",
