...
Every form on the site (`ContactCTA.astro`, Load Board, logistics apply/intake) defaults to **preview mode**: no network request, no storage, no automatic delivery. A visitor gets a generated email/summary they can copy or open in their own mail client. Live delivery is normally opt-in per environment via `PUBLIC_CONTACT_MODE=live` + `PUBLIC_CONTACT_ENDPOINT` (see `.env.example`). The one bounded exception is the direct car-hauling dispatch intake: after the production receiver and its bindings were independently verified live, `CarrierDispatchIntakeEnhancer.astro` promotes that form to the existing same-origin `/api/logistics-lead` receiver only when the browser is on canonical `https://hermeslogisticsus.com`; local development and preview domains remain preview-only. Do not broaden this exception to other forms or hosts without equivalent receiver, privacy, idempotency, rate-limit, delivery-receipt, and production evidence. This is a deliberate, audited product contract — see `docs/DESIGN_INTEGRATION_CONTRACT.md`.
...
`wrangler.jsonc.example` is a versioned runtime reference only. There is **no active root Wrangler configuration** in the public repository, and agents must not create an untracked root `wrangler.toml`/`wrangler.jsonc` as a second Pages deployment authority. Current Production and Preview runtime configuration is owned by Cloudflare Pages project state; authenticated API readback on 2026-09-18 showed `wrangler_config_hash = null`, so no active Wrangler-file runtime authority exists. Do not copy production resource IDs or secret values into the repository. The contact receiver remains covered by `scripts/sales-lead-receiver.test.mjs`.
